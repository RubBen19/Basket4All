...
@Composable
fun FaultsPopUp(viewModel: AddPlayerScreenViewModel) {
    // Inputs
    ...
    AlertDialog(
        ...
        onDismissRequest = { viewModel.hide("Faltas") },
        confirmButton = {
            IconButton(
                onClick = {
                    val faults = viewModel.matchStats.value?.faults
                    faults?.setInFaults(inside.value.toIntOrNull() ?: 0)
                    faults?.setOutFaults(outside.value.toIntOrNull() ?: 0)

                    viewModel.hide("Faltas")
                }
            ) {
                Icon(
