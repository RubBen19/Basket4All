...
@Composable
fun ReboundsPopUp(viewModel: AddPlayerScreenViewModel) {
    // Inputs
    ...
    AlertDialog(
        ...
        onDismissRequest = { viewModel.hide("Rebotes") },
        confirmButton = {
            IconButton(
                onClick = {
                    val rebounds = viewModel.matchStats.value?.rebounds
                    rebounds?.setOfRebound(offensive.value.toIntOrNull() ?: 0)
                    rebounds?.setDefRebound(defensive.value.toIntOrNull() ?: 0)

                    viewModel.hide("Rebotes")
                }
            ) {
                Icon(
