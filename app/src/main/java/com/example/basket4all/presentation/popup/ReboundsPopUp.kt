...
@Composable
fun ReboundsPopUp(viewModel: AddPlayerScreenViewModel, screenUiState: AddPlayerScreenUiState) {
    // Inputs
    val offensive = remember { mutableStateOf("") }
    val defensive = remember { mutableStateOf("") }

    AlertDialog(
        modifier = Modifier.padding(top = 20.dp, bottom = 32.dp),
        onDismissRequest = { viewModel.show("Rebotes") },
        confirmButton = {
            IconButton(
                onClick = {
                    val rebounds = screenUiState.matchStats.rebounds
                    rebounds.setOfRebound(offensive.value.toIntOrNull() ?: 0)
                    rebounds.setDefRebound(defensive.value.toIntOrNull() ?: 0)

                    viewModel.show("Rebotes")
                }
            ) {
                Icon(
        ...
        text = {
            SuccessAndFails(offensive, defensive)
        }
    )
}
