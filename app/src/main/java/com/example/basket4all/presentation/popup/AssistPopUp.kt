...
@Composable
fun AssistPopUp(viewModel: AddPlayerScreenViewModel) {
    // Inputs
    ...
    AlertDialog(
        ...
        onDismissRequest = { viewModel.hide("Asistencias") },
        confirmButton = {
            IconButton(
                onClick = {
                    val lastPasses = viewModel.matchStats.value?.lastPass
                    lastPasses?.setAssist(success.value.toIntOrNull() ?: 0)
                    lastPasses?.setNoAssist(failed.value.toIntOrNull() ?: 0)
                    viewModel.hide("Asistencias")
                }
            ) {
                Icon(
