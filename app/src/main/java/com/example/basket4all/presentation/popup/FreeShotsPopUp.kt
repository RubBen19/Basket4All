...
@Composable
fun FreeShotsPopUp(viewModel: AddPlayerScreenViewModel, screenUiState: AddPlayerScreenUiState) {
    // Inputs
    ...
    AlertDialog(
        ...
        onDismissRequest = { viewModel.show("Tiros libres") },
        confirmButton = {
            IconButton(
                onClick = {
                    val freeShots = screenUiState.matchStats.shots.getFreeShots()
                    freeShots.setTotal(
                        success.value.toIntOrNull() ?: 0,
                        failed.value.toIntOrNull() ?: 0
                    )
                    viewModel.show("Tiros libres")
                }
            ) {
                Icon(
