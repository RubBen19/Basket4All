...
@Composable
fun FreeShotsPopUp(viewModel: AddPlayerScreenViewModel) {
    // Inputs
    ...
    AlertDialog(
        ...
        onDismissRequest = { viewModel.hide("Tiros libres") },
        confirmButton = {
            IconButton(
                onClick = {
                    val freeShots = viewModel.matchStats.value?.shots?.getFreeShots()
                    freeShots?.setTotal(
                        success.value.toIntOrNull() ?: 0,
                        failed.value.toIntOrNull() ?: 0
                    )
                    viewModel.hide("Tiros libres")
                }
            ) {
                Icon(
